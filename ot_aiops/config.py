"""Configuration management for ot-aiops.

Loads OT endpoint targets from a YAML config file (``~/.ot-aiops/config.yaml``).
...
encrypted store ``~/.ot-aiops/secrets.enc`` (see
:mod:`ot_aiops.secretstore`), keyed by the endpoint target name.
...
from ot_aiops.secretstore import (
    SecretStoreError,
    get_secret,
    has_store,
)

CONFIG_DIR = Path.home() / ".ot-aiops"
...
def _load_env() -> None:
    """Load ~/.ot-aiops/.env so legacy per-endpoint passwords are available."""
    if ENV_FILE.exists():
...
def _resolve_secret(name: str) -> str:
    ...
    ``ot-aiops doctor``, not a hard error.
    """
    if has_store():
        try:
    ...
    def default_target(self) -> TargetConfig:
        if not self.targets:
            raise ValueError(
                ...
                "~/.ot-aiops/config.yaml (or run 'ot-aiops init')."
            )
