"""``ot-aiops init`` — a friendly, interactive onboarding wizard.
...
from ot_aiops.secretstore import SecretStore, resolve_master_password
...
def init_cmd() -> None:
    ...
    console.print(
        "[dim]Encrypts secrets.enc. Set it via OT_AIOPS_MASTER_PASSWORD for "
    ...
    console.print(
        "[dim]Tip: export OT_AIOPS_MASTER_PASSWORD=... so the MCP server and CLI "
        "can unlock secrets non-interactively.[/]"
    )
    if typer.confirm("Run a connectivity check now (ot-aiops doctor)?", default=True):
        from ot_aiops.doctor import run_doctor
