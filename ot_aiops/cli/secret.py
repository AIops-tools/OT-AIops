"""``ot-aiops secret`` — manage the encrypted credential store.

Secrets are stored in ``~/.ot-aiops/secrets.enc`` (Fernet, key derived from a
...
from ot_aiops.cli._common import cli_errors, console
from ot_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: ot-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update OT_AIOPS_MASTER_PASSWORD.[/]")
