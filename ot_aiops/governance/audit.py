"""Unified audit logging engine — all ot-aiops tools write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.ot-aiops/audit.db``.
...
from ot_aiops.governance.paths import ops_path

_log = logging.getLogger("ot-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring OT_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the OT_AIOPS_HOME refactor
