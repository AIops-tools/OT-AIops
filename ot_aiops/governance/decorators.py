...
Responsibilities:
  ...
  3. Post-log: write audit record to ``~/.ot-aiops/audit.db``
...
from ot_aiops.governance.audit import detect_agent, get_engine
from ot_aiops.governance.budget import BudgetExceeded, get_budget
from ot_aiops.governance.patterns import PatternMatch, get_pattern_engine
from ot_aiops.governance.policy import PolicyResult, get_policy_engine
from ot_aiops.governance.sanitize import sanitize

_log = logging.getLogger("ot-aiops.decorators")
    ...
    *,
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.ot-aiops/undo.db and the
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    if not descriptor:
        return
    try:
        from ot_aiops.governance.undo import get_undo_store
...
def _infer_skill(func: Any) -> str:
    ...
    ``ot_aiops.ops.jobs`` → ``ot-aiops``
    ``mcp_server.server`` → ``ot-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("ot_aiops") or module.startswith("mcp_server"):
        return "ot-aiops"
    return "unknown"
