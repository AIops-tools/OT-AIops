"""Encrypted secret storage for ot-aiops.

Secrets (per-endpoint OPC-UA / Modbus passwords) are NEVER written to disk in
plaintext. They live in ``~/.ot-aiops/secrets.enc`` encrypted with Fernet
...
The master password is resolved (in order) from:
  ...
  1. the env var ``OT_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
MASTER_PASSWORD_ENV = "OT_AIOPS_MASTER_PASSWORD"  # nosec B105 — env var name, not a secret
# ──────────────────────────────────────────────────────────────────────────────
