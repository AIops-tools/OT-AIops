...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "troubleshooting (preview). Protocols: OPC-UA, Modbus-TCP, S7comm "
        "(Siemens + 仿西门子 国产), Mitsubishi MC (Q/L/iQ-R), MTConnect (CNC "
        "machine tools), and MQTT/Sparkplug B (UNS); EtherNet/IP + EtherCAT are "
        "documented roadmap stubs. Call protocols_supported() for the capability "
        ...
        "anomaly offenders), historian_health (gap/flatline over a series). A few "
        "WRITE/command tools (s7_write_db, mc_write_words, mqtt_publish) are "
        "OT-dangerous: governed at HIGH risk_tier, off by default (dry-run), "
        "capture the BEFORE value for undo, and need a recorded approver (MOC). "
